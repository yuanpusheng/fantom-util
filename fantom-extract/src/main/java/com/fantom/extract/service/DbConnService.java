...
public class DbConnService {
    ...
    public Integer update(DbConn dbConn) {
        Map<String, ?> params = BeanMap.create(dbConn);
        params.entrySet().stream().filter(entry -> !ObjectUtils.isEmpty(entry.getValue()));
        String preSql = SqlUtil.genInsertPreSql("db_conn", params);
        return namedParameterJdbcTemplate.update(preSql, new BeanPropertySqlParameterSource(dbConn));
    }

    public List<Map<String, Object>> list() {
        String preSql = "SELECT * FROM db_conn";
